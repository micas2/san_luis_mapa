// Genera el SQL INSERT para los 73 lotes faltantes
...
let sql = `-- ============================================================\n`;
sql += `-- INSERT de ${missing.length} lotes faltantes en san_luis\n`;
sql += `-- Proyecto: SAN LUIS CHICO\n`;
sql += `-- Los campos Superficie, Precio, Orientacion, Precio_financiado, Cuota\n`;
sql += `-- quedan en NULL para que los completes luego.\n`;
sql += `-- ============================================================\n\n`;

sql += `INSERT INTO public.san_luis\n`;
sql += `  ("Proyecto", "ID", "Lote", "Manzana", "Superficie", "Estado", "Precio", "Orientacion", "Precio_financiado", "Cuota", "Uso")\nVALUES\n`;

const rows = missing.map(([id, manzana, lote, uso]) => {
  return `  ('SAN LUIS CHICO', '${id}', '${lote}', '${manzana}', NULL, 'DISPONIBLE', NULL, NULL, NULL, NULL, '${uso}')`;
});

sql += rows.join(',\n') + '\n';
sql += `ON CONFLICT ("ID") DO NOTHING;\n\n`;

sql += `-- Verificar cuántos se insertaron:\n`;
sql += `SELECT COUNT(*) FROM public.san_luis;\n`;
